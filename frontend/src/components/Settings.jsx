...
import { getConfig, saveConfig, uploadCookies, testTelegram, clearLeads } from '../services/api';
...
const QTY_PRESETS = [0, 10, 50, 100, 500, 1000];
...
export default function Settings() {
  ...
  const addKeyword = (e) => {
    ...
    if (kw && !cfg.keywords.includes(kw)) {
      set('keywords', [...cfg.keywords, kw]);
      setKwInput('');
    }
  };
  ...
  return (
    <div className="settings-page">
      ...
      <div className="glass-panel settings-section">
        ...
        <p className="section-desc">Paste JSON from EditThisCookie extension or a raw cookie string from DevTools.</p>
        <textarea
          rows="5"
        ...
        />
        <button id="upload-cookies-btn" className="btn btn-primary" style={{ marginTop: '0.75rem' }} onClick={handleUploadCookies}>
          📤 Upload & Save Cookies
        </button>
      </div>
      ...
      <div className="glass-panel settings-section">
        ...
        <form onSubmit={addKeyword} style={{ display: 'flex', gap: '0.75rem', marginBottom: '0.75rem' }}>
          <input type="text" placeholder="e.g. Paracetamol, Amoxicillin…" value={kwInput} onChange={e => setKwInput(e.target.value)} />
      ...
      <div className="glass-panel settings-section">
        ...
        <div className="qty-presets">
          {QTY_PRESETS.map(q => (
            <button
              key={q}
              className={`btn ${cfg.min_quantity === q ? 'btn-primary' : 'btn-outline'}`}
              onClick={() => set('min_quantity', q)}
            >
              {q === 0 ? 'No Limit' : `${q}+`}
            </button>
          ))}
        ...
        <input
          type="number" min="0"
          value={cfg.min_quantity || 0}
          onChange={e => set('min_quantity', parseInt(e.target.value) || 0)}
          style={{ marginTop: '0.75rem', maxWidth: '160px' }}
      ...
      <div className="glass-panel settings-section">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <input
            type="number" min="1"
            value={cfg.accept_limit || 100}
            onChange={e => set('accept_limit', parseInt(e.target.value) || 1)}
      ...
      <div className="glass-panel settings-section">
        ...
        <label className="toggle-row">
          ...
          <div
            className={`toggle ${cfg.reply_enabled ? 'on' : 'off'}`}
            onClick={() => set('reply_enabled', !cfg.reply_enabled)}
          />
        </label>
        <p className="section-desc" style={{ marginTop: '1rem' }}>
          Message template — use <code style={{ color: '#a78bfa' }}>{'{name}'}</code>, <code style={{ color: '#a78bfa' }}>{'{product}'}</code>, <code style={{ color: '#a78bfa' }}>{'{company}'}</code> as placeholders.
        </p>
        <textarea
          rows="4"
          value={cfg.auto_reply_msg || ''}
          onChange={e => set('auto_reply_msg', e.target.value)}
          placeholder="Thank you for your inquiry about {product}. We will get back to you shortly."
        />
      </div>
      ...
      <div className="glass-panel settings-section">
        ...
        <button className="btn btn-outline" style={{ marginTop: '0.75rem' }} onClick={handleTelegramTest} disabled={tgTesting}>
